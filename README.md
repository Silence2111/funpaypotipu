...
| 12 | [Деплой](docs/12-deploy.md) | k8s-манифесты, prod-compose, CI/CD, чек-лист |
...
### Вариант A — весь стек в Docker (одна команда)

```bash
cp .env.example .env               # секреты (JWT_*, DATA_ENCRYPTION_KEY)
make up                            # собрать и поднять БД + api + web + worker + миграции
# web → http://localhost:3000 · api → http://localhost:4000/api · MinIO → :9001 · Mail → :8025
```

`make up` = `docker compose -f infra/compose.yaml up -d --build`. Одноразовый сервис
`migrate` применяет миграции и сид, затем `api`/`web`/`worker` стартуют. Прочие команды:
`make logs`, `make ps`, `make down`, `make restart`.

### Вариант B — локальная разработка (Node на хосте)

Требуется Node.js ≥ 22 и pnpm ≥ 9 (`corepack enable pnpm`).

```bash
pnpm install
cp .env.example .env
docker compose -f infra/compose.yaml up -d postgres redis minio meilisearch mailhog
pnpm db:generate && pnpm db:migrate && pnpm db:seed
pnpm dev                           # api :4000, web :3000, worker
```

Проверка: `GET http://localhost:4000/api/health` → `{ status, db, ts }`.

> Без Docker БД можно поднять через `embedded-postgres` (user-space бинарь) — так
> проверялся сквозной сценарий. Prod-путь — managed PostgreSQL.
